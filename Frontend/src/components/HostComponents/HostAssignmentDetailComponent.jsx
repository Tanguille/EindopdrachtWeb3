                    ...
                    <tbody>
                        {subAssignments.map((subAssignment, index) => (
                            <tr key={index}>
                                ...
                                <td className="text-center text-gray-600 mb-1 px-4 py-2"><Countdown key={assignment.id} ref={countdownRef} date={Date.now() + (subAssignment.minuten * 1000 * 60)} autoStart={false} renderer={renderer} /></td>
                                <td className="text-center text-gray-600 mb-1 px-4 py-2">{subAssignment.extraMinuten}</td>
                                <td className="px-4 py-2">
                                    <button onClick={() => navigate("rapporten")}>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
